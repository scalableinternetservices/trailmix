...
usage="Usage: deploy-ecs.sh [trailmix-app-web|trailmix-app-background] [sha]"
...
new_image="101624687637.dkr.ecr.us-west-2.amazonaws.com/trailmix:$2"
...
aws ecs update-service --cluster trailmix \
  --no-cli-pager \
  --region "$ecs_region" \
